...
```mermaid
flowchart TB
    L4[Contract tests — Bruno/bru CLI against a running API]
...
- **Contract tests** — the Bruno collection (docs/API_SPEC.md, `bruno/`) run via the `bru` CLI (`@usebruno/cli`) against a running stack as a regression check. If the collection and the spec disagree, that's a bug in one of them, not a reason to skip the check.
